...
class BarangKeluarController extends Controller
{
    ...
    public function update(Request $request, string $id)
    {
        ...
        try {
            // Ambil data barang_keluar yang akan diupdate
            ...
            if (!$barang) {
                return redirect()->route('barang-keluar.index')->withErrors('Barang terkait tidak ditemukan.');
            }

            // Kembalikan stok barang ke kondisi sebelum entry ini
            $barang->stok += $barangKeluar->qty_keluar;

            // Pastikan barang yang dipilih masih tersedia dan stok mencukupi setelah pengurangan
            if ($request->barang_id != $barangKeluar->barang_id) {
                // Jika barang_id berubah, kurangi stok barang baru
                $barangBaru = BarangModel::find($request->barang_id);
                if (!$barangBaru || $barangBaru->stok < $request->qty_keluar) {
                    return redirect()->back()->with('error', 'Stok barang baru tidak mencukupi!');
                }
                $barangBaru->stok -= $request->qty_keluar;
                $barangBaru->save();
            } else {
                // Jika barang_id tidak berubah, perbarui stok yang sama
                if ($barang->stok < $request->qty_keluar) {
                    return redirect()->back()->with('error', 'Stok barang tidak mencukupi!');
                }
