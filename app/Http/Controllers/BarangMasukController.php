...
class BarangMasukController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return view('backend.barang-masuk.index', compact('rsMasuk'));
    }
    ...
    {
        $listBarang = BarangModel::select(DB::raw("CONCAT(merk, ' - ', seri) AS nama_barang"), 'id')->pluck('nama_barang', 'id');

        return view('backend.barang-masuk.create', compact('listBarang'));
    }
    ...
    public function store(Request $request)
    {
        ...
        $existingData = BarangMasuk::where('barang_id', $request->barang_id)
        ...
        // ->increment('stok', $request->qty_masuk);  // Tambahkan qty_masuk ke stok
    ...
    public function show(string $id)
    {
        ...
        return view('backend.barang-masuk.show', compact('rsMasuk'));
    }
    ...
    public function edit(string $id)
    {
        ...
        return view('backend.barang-masuk.edit', compact('rsMasuk', 'listBarang'));
    }
    ...
    public function update(Request $request, string $id)
    {
        ...
        try {
            ...
            if (!$barangMasuk) {
                return redirect()->route('barang-masuk.index')->withErrors('Data barang masuk tidak ditemukan.');
            }

            // Ambil data barang terkait
            $barang = DB::table('barang')->where('id', $request->barang_id)->first();
            if (!$barang) {
                return redirect()->route('barang-masuk.index')->withErrors('Barang tidak ditemukan.');
            }
            ...
            DB::rollBack();
            return redirect()->route('barang-masuk.index')->withErrors('Terjadi kesalahan: ' . $e->getMessage());
        }
    }
    ...
    public function destroy(string $id)
    {
        ...
        $rowMasuk = BarangMasuk::orderBy('id')->get();  // Ambil semua data barang yang tersisa
        $newId = 1;
